/* VPAT prototype data — modeled on VPAT® 2.5Rev International Edition (Feb 2025).
   Three reports in one ACR: WCAG 2.x · Revised Section 508 · EN 301 549. */
(function () {
  const CONF = {
    SUPPORTS: "Supports",
  ...
  const REPORTS = [
    { id: "wcag", name: "WCAG 2.2 Report", tag: "Tables 1–2",
      ...
      sections: [ { id: "A", name: "Table 1 — Success Criteria, Level A" }, { id: "AA", name: "Table 2 — Success Criteria, Level AA" } ] },
    { id: "508", name: "Revised Section 508 Report", tag: "Ch. 3–6",
      ...
      sections: [ { id: "fpc", name: "Chapter 3 — Functional Performance Criteria" }, { id: "ref508", name: "Chapters 4–6 — Hardware, Software & Docs" } ] },
    { id: "en", name: "EN 301 549 Report", tag: "Clauses 4–13",
      ...
      sections: [ { id: "fps", name: "Clause 4 — Functional Performance Statements" }, { id: "refen", name: "Clauses 5–13 — Generic, Hardware, Web & Docs" } ] },
  ];

  // ---- WCAG 2.x success criteria (Level A + AA), with AI-drafted findings ----
  // ver: WCAG version that introduced the criterion. appliesTo computed in UI.
  const WCAG = [
  ...
  ].map(c => ({ ...c, report: "wcag", section: c.level }));

  // Note row for the obsolete 4.1.1 Parsing criterion (removed in WCAG 2.2)
  const PARSING_NOTE = { id: "4.1.1", name: "Parsing (obsolete)", level: "A", ver: "2.0/2.1", principle: "Robust",
  ...
  ].map(c => ({ ...c, report: "508", section: "fpc", auto: c.auto || 0, principle: "Functional Performance" }));
  ...
  ].map(c => ({ ...c, report: "en", section: "fps", auto: 0, evidence: c.evidence || [], principle: "Functional Performance" }));
  ...
  const CRITERIA = [...WCAG, PARSING_NOTE, ...FPC_508, ...FPS_EN];

  window.VPAT = { CONF, TERMS, STANDARDS, PAGES, SCAN_PHASES, GEN_PHASES, REPORTS, CRITERIA, AUTO,
    VERSION: "VPAT\u00ae 2.5Rev", EDITION: "International Edition" };
})();
